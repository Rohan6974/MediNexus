...
export async function getDoctorAvailability() {
  ...
  try {
    ...
    console.log(`Failed to get doctor availability ${error.message}`);
  }
}
...
export async function addAppointmentNotes(formData) {
  ...
  try {
    ...
    throw new Error("Failed to update notes", error || error.message);
  }
}
...
export async function markAppointmentCompleted(formData) {
  ...
  try {
    ...
    if (now < appointmentEndTime) {
      throw new Error(
        "cannot mark appointment as completed gefore the scheduled end time"
      );
    }
